...
//你的 excel 檔案路徑 (含檔名)
$inputFileName = './products0103.xlsx';
...
//讀取當前工作表(sheet)的資料列數
$sheet = $spreadsheet->getSheetByName('relative');
// print_r($sheet);
...
for ($i = 2; $i <= $highestRow; $i++) {
    ...
    $cat_id = $sheet->getCell("A" . $i)->getValue();
    $cat_name = $sheet->getCell("B" . $i)->getValue();
    $prod_id = $sheet->getCell("C" . $i)->getValue();
    $prod_name = $sheet->getCell("D" . $i)->getValue();
    ...
    $sql = "INSERT INTO `relative`(`cat_id`, `cat_name`, `prod_id`, `prod_name`)
            VALUES ({$cat_id},'{$cat_name}', {$prod_id}, '{$prod_name}')";
...
//上傳時在ターミナル輸入php .\convert0105.php
